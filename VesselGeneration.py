...
####################################################################################################################
def AddVessel(VesselName="Vessel",ContentName="MaterialContent",MinH=16,MaxH=80,MinR=4,MaxR=40,ScaleFactor=0.1,SimpleLiquid=True):   
...
#--------------------------------------------------------------------------------------
    ...
    while(VesselWallThikness>np.max(rl)):
        if np.random.rand()<1:
              VesselWallThikness=(np.max(rl)/(12))*(np.random.rand()*np.random.rand()+0.1)
        else:
    ...
    for fz in range(len(rl)):
        for j in range(0,Vnum ):
         ...
         #   print(x)
            ...
            Mvert = (x* MatX_RadRatio-VesselWallThikness* math.cos(theta)*1.03,y* MatY_RadRatio-VesselWallThikness* math.sin(theta)*1.03,z)  # 
...
#*******************************************************************************
    ...
    if Smooth: bpy.ops.object.shade_smooth() # smooth 
    #===================================================================================
    print("44444444444444444444444ddddddddddddddddddddddddddddddddddddddddddd")
    return MaxXY,MaxZ,VesselFloorHeight,VesselWallThikness,MaterialTopHeight